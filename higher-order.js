...
let postTaxPrices = prices.map((num) => {return num * 1.07;})
...
//Code Here
let myStrongest = monstersInYourPocket.filter((x) => {return x["CP"]>200})
...
let ordersTotal = orders.map(x => {return x["price"] * x["tax"] + x["price"]})
...
let bobsTotal = purchases.reduce((purchases) => {
  var total = 0;
